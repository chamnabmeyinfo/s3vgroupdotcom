-- S3V Trading Group - Clean Seed (safe to run multiple times)
-- This uses IF NOT EXISTS and ON CONFLICT to avoid errors
...
-- Create Product Media Table (if not exists)
...
-- Create Portfolio Projects Table (if not exists)
...
-- Insert sample data (uses ON CONFLICT to prevent duplicates)
...
-- Now insert products (categories must exist first)
INSERT INTO "products" ("id", "name", "slug", "sku", "summary", "description", "category_id", "price", "status", "highlights", "hero_image", "specs") VALUES
  (
    'prod-1',
    'Heavy Duty Truck Scale 100T',
